...
import static net.phazoganon.froghopperboots.util.ModUtils.addEntityHandler;
...
public class OnFallEvent {
    ...
    public static void onFall(LivingFallEvent event) {
        ...
        if (livingEntity.fallDistance > 0) {
            if (livingEntity.getItemBySlot(EquipmentSlot.FEET).is(ModItems.FROG_HOPPER_BOOTS)) {
                if (!livingEntity.isSuppressingBounce()) {
                    ...
                    if (livingEntity instanceof ServerPlayer) {
                        ...
                        livingEntity.setDeltaMovement(vec3.x/0.975F, velocity, vec3.z/0.975F);
                        livingEntity.hurtMarked = true;
                        addEntityHandler(livingEntity);
                    }
                    else {
                        ...
                        ModUtils.addEntityHandler(livingEntity, livingEntity.getDeltaMovement());
                    }
